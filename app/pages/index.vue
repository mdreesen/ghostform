  ...
  <div class="min-h-screen bg-zinc-950 text-white flex items-center justify-center p-6 font-sans">
    <div v-if="!aiResult" class="max-w-md w-full space-y-8">
      <div class="h-1 bg-zinc-800 rounded-full">
        <div class="h-1 bg-blue-500 transition-all duration-500" :style="{ width: `${((step + 1) / questions.length) * 100}%` }"></div>
      </div>
      ...
      <button v-if="!isSubmitting" @click="nextStep" class="bg-blue-600 px-6 py-2 rounded-lg hover:bg-blue-500 transition">
        {{ step === questions.length - 1 ? 'Finish' : 'Next' }}
      </button>
    </div>

    <div v-else class="bg-zinc-900 p-8 rounded-2xl border border-zinc-800 max-w-lg">
      <h2 class="text-green-400 font-bold mb-2">AI Analysis Complete</h2>
      <p class="text-zinc-400 mb-4 italic">"{{ aiResult.suggestedReply }}"</p>
      <div class="flex gap-4">
        <span class="px-3 py-1 bg-zinc-800 rounded text-sm capitalize">Priority: {{ aiResult.priority }}</span>
      </div>
    </div>
  </div>
</template>
